...
const ContractChart: React.FC<ContractChartProps> = ({ contracts, selectedYear }) => {
  ...
  return (
    <>
      ...
      <div className="flex justify-around items-end h-56 space-x-2 text-center" aria-label={selectedYear === 'all' ? "Grafico a barre dei contratti per anno" : `Grafico a barre dei nuovi contratti per l'anno ${selectedYear}`}>
        {chartData.length > 0 ? (
          chartData.map(({ label, count }) => (
            <div key={`${label}-${selectedYear}`} className="flex flex-col items-center justify-end w-full h-full group">
              ...
              <div
                className="w-3/4 bg-green-300 hover:bg-green-500 dark:bg-green-700 dark:hover:bg-green-500 rounded-t-lg transition-all duration-500 ease-out cursor-pointer relative"
                style={{ height: `${(count / maxCount) * 100}%` }}
                role="img"
                aria-label={`Barra per ${label}: ${count} contratti`}
              >
                 <div className="sr-only">{`${label}: ${count} contratti`}</div>
              </div>
              <div className={`mt-3 text-xs font-medium text-slate-500 dark:text-slate-400 uppercase ${selectedYear === 'all' ? '' : '[writing-mode:vertical-rl] [transform:rotate(180deg)]'}`}>
                {label}
              </div>
            </div>
          ))
