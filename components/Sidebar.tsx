...
import { ChartBarIcon, UserGroupIcon, DocumentDuplicateIcon, XIcon, SunIcon, MoonIcon, LogoutIcon, CogIcon, BriefcaseIcon, ClipboardListIcon, CalculatorIcon } from './Icons';

type View = 'dashboard' | 'contracts' | 'clients' | 'appointments' | 'office-tasks' | 'bill-splitter' | 'settings';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, expiringContractsCount, isOpen, onClose, onLogout, theme, onThemeSwitch }) => {
  return (
    <>
        {/* Overlay for mobile (hidden on desktop via lg:hidden) */}
        <div
        ...
        {/* Sidebar Panel */}
        {/* Removed 'lg:translate-x-0' to allow toggling on desktop via the isOpen prop */}
        <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white dark:bg-slate-800 h-screen flex flex-col shadow-lg border-r border-slate-200 dark:border-slate-700 flex-shrink-0 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
